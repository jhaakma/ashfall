...
---@class Ashfall.AttachNodeConfig
---@field attachNodeName string The name of the attach node on the campfire mesh
---@field getDoAttach fun(campfire: tes3reference): boolean A function that returns true if the item should be attached based on the campfire's current state
---@field getAttachMesh fun(campfire: tes3reference): niNode|nil  A function that returns the mesh to be attached to the campfire
---@field postAttach? fun(campfire: tes3reference, attachNode: niNode) A function that is called after the attach node has been attached to the campfire, for any additional setup (like adding patina)

--Note: these must be ordered correctly, for example, cooking pot comes before ladle
---@type Ashfall.AttachNodeConfig[]
local attachNodes = {

    {
        --Will place SWITCH_FIRE node for fire particles
        attachNodeName = "ATTACH_FIRESWITCH",
        getDoAttach = function(campfire)
            return true
        end,
        getAttachMesh = function(campfire)
            return common.helper.loadMesh("ashfall\\anim\\switch_fire.nif")
        end,
    },

    {
        --deprecated, use ATTACH_FIRESWITCH
...
local switchNodeValues = {
    ...
    SWITCH_KILN = function(reference)
        local isOpen = reference.data.kilnOpen
...
---@class Ashfall.ProcessNodeConfig
---@field nodeName string The name of the node to be processed
---@field process fun(node: niNode, reference: tes3reference) A function that takes in the node and reference and processes the node in some way (for example, changing the position based on fuel level)

local ashTextures= {
    unlit = niSourceTexture.createFromPath("textures/tx_ash_firepit.dds"),
    hot = niSourceTexture.createFromPath("textures/ashfall/campfire/coal_r.dds"),
    cold = niSourceTexture.createFromPath("textures/ashfall/campfire/coal_b.dds"),
}

---@type Ashfall.ProcessNodeConfig[]
local processNodes = {
    {
        nodeName = "ASHFALL_ASHES",
        process = function(node, reference)
            logger:trace("Processing node %s on reference %s", node.name, reference)
            if not node.texturingProperty then
                logger:debug("Node %s has no texturing property, skipping", node.name)
                return
            end
            if not node.texturingProperty.baseMap then
                logger:debug("Node %s has no base map, skipping", node.name)
                return
            end
            ...
            local newTexture = ashTextures.unlit
            if isLit then

                newTexture = isCold and ashTextures.cold or ashTextures.hot
            end
            node.texturingProperty = node.texturingProperty:clone()
            node.texturingProperty.baseMap.texture = newTexture
            node:updateProperties()
            logger:trace("Updated texture on node %s to %s", node.name, newTexture.fileName)
        end
    }
}
...
local function updateProcessNodes(reference)
    logger:trace("Ashfall:UpdateProcessNodes: %s", reference.object.id)
    for _, config in ipairs(processNodes) do
        local node = reference.sceneNode:getObjectByName(config.nodeName)
        if node then
            logger:trace("Processing node %s on reference %s", node.name, reference)
            config.process(node, reference)
        end
    end
end

    ---@param reference tes3reference
local function updateAttachNodes(reference)
    logger:trace("Ashfall:UpdateAttachNodes: %s", reference.object.id)

    local sceneNode = reference.sceneNode
    if not reference.data then
        logger:trace("Reference has no data, skipping attach nodes")
        return

    end
    if not sceneNode then
        logger:trace("Reference has no scene node, skipping attach nodes")
        return
    end
...
local function updateCampfireVisuals(campfire)
    ...
    logger:trace("Finished updating visuals for %s", campfire)
end
...
event.register("Ashfall:UpdateAttachNodes", function(e)
    if not e.reference then return end
    updateCampfireVisuals(e.reference)
end)
